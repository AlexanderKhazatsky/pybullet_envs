...
class VALPotV0(VALBaseEnvV0):
    ...
    def get_info(self):
        delta_pos = [-3.38739228e-05, -4.18773970e-02, 0]
    ...
    def demo_reset(self):
        ...
        self.done = False
        self.grip = -1.
        self.default_height = self.get_end_effector_pos()[2] - 0.02
        return reset_obs
    ...
    def move_obj(self):
        delta_pos = [-6.38739228e-05, -4.18773970e-02, 0]
        # delta_pos = [-3.38739228e-05, -4.18773970e-02, 0]
        goal_pos = self.get_object_pos('pot') + delta_pos

        ee_pos = self.get_end_effector_pos()
        obj_pos = self.get_object_pos('lid')
        target_pos = obj_pos + np.array([0., 0., 0.035])
        ...
        drop_object = np.linalg.norm(obj_pos[:2] - goal_pos[:2]) < 0.02
    ...
    def _reset_scene(self):
        ...
        # while True:
        #     pot_pos = np.array([np.random.uniform(.45, .55),
        #        np.random.uniform(-0.1, -0.05),
        #        -.2])

        #     lid_pos = np.array([np.random.uniform(.65, .75),
        #        np.random.uniform(-0.1, -0.05),
        #        -.2])

        #     print('here')
        #     if np.linalg.norm(pot_pos[0] - lid_pos[0]) > 0.15:
        #         break
        ...
        self._objects['lid'] = bullet.objects.big_lid(
            pos=lid_pos,
            scale=0.082,
            rgba=[.815, .84, .875, 1],
            )
        ...
        pos = np.array([np.random.uniform(.45, .55),
